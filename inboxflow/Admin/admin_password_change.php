...
$username = isset($_SESSION['username']) ? $_SESSION['username'] : header("location:admin_login.php");
?>
<!DOCTYPE html>
<html lang="en">
                        ...
                        if ($get_query_output->num_rows > 0) {
                            ...
                            if ($temporary_code === $result['otp']) {
                                if (!empty($_POST['password']) && preg_match("/^[a-zA-Z0-9@_. ]*$/", $_POST['password'])) {
                                    ...
                                    if (!empty($_POST['confirm_password']) && preg_match("/^[a-zA-Z0-9@_ .]*$/", $_POST['confirm_password'])) {
                                        ...
                                        if ($password === $confirm_password) {
                                            ...
                                            $password_update_query = "update admin_details set password='$hash_password', password_attempt='4',forgot_pass_attempt='2',token_attempt='1',otp=NULL where username='$username';";
                                ...
                                if ($forgot_pass_attempt > 0) {
                                    $forgot_attempt_update = "update admin_details set forgot_pass_attempt='$forgot_pass_attempt' where username='$username';";
                                    ...
                                    header("location:admin_token_verify.php");
                                }
                            }
                        }
                    }
